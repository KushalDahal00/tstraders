...
import { getCategories, saveProduct } from '@/lib/store';
...
export default function NewProductPage() {
  ...
  const [brand, setBrand] = useState('T.S Traders Signature');
  const [categoryId, setCategoryId] = useState('');
  const [sku, setSku] = useState(`TS-${Date.now().toString().slice(-6)}`);
  const [description, setDescription] = useState('');
  const [price, setPrice] = useState<number | ''>(14999);
  const [discountPrice, setDiscountPrice] = useState<number | ''>('');
  const [mainImage, setMainImage] = useState('https://images.unsplash.com/photo-1542291026-7eec264c27ff?auto=format&fit=crop&w=1000&q=80');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!mainImage) {
      setErrorMessage('Product must have a main image');
      return;
    }
  ...
  return (
    <div className="flex-1 space-y-8 pb-20">
      <AdminHeader
        ...
        subtitle="Create a new shoe listing, configure sizes, stock counts, and upload product images"
      />
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          ...
          <div className="bg-[#141417] p-6 border border-zinc-800 space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <input
                  ...
                  placeholder="e.g. Nike Air Max 270 Supreme"
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="e.g. Nike, Adidas, T.S Traders Signature"
                  className="w-full px-3 py-2 bg-zinc-900 border border-zinc-800 text-xs text-white focus:outline-none focus:border-zinc-500"
                />
              </div>
            ...
            <div>
              ...
              <textarea
                ...
                placeholder="Comfortable everyday sneakers designed for casual and active use..."
          ...
          <div className="bg-[#141417] p-6 border border-zinc-800 space-y-4">
            ...
            <div className="space-y-3">
              <label className="block text-xs font-semibold uppercase tracking-wider text-zinc-300">
                Add Image URL (Unsplash or Supabase Storage URL)
              </label>
          ...
          <div className="bg-[#141417] p-6 border border-zinc-800 space-y-4">
            <div className="flex items-center justify-between border-b border-zinc-800 pb-3">
              ...
              <span className="text-xs text-zinc-400 font-mono">Size Matrix</span>
            </div>

            <p className="text-xs text-zinc-400">
              Select available shoe sizes and specify exact stock counts for each size. Setting size stock to 0 triggers "Limited Availability".
            </p>
            ...
            {/* Per-Size Numerical Stock Input Grid */}
            {sizesState.length > 0 && (
          ...
          <div className="flex items-center justify-end space-x-4">
            ...
            >
              {saving ? 'Saving Product...' : 'Publish Product To Store'}
            </button>
          </div>
        </form>
      </div>
